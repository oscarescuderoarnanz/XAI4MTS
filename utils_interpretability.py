...
def plot1_metrics_over_time(all_metrics, T):
    """
    Plot metrics over time with error bars indicating standard deviation.
    Args:
    ...
    plt.errorbar(metrics_mean.index, metrics_mean['Specificity'], yerr=metrics_std['Specificity'], label='Specificity', marker='o', capsize=5)
    plt.errorbar(metrics_mean.index, metrics_mean['Recall'], yerr=metrics_std['Recall'], label='Sensitivity', marker='s', capsize=5)
    plt.errorbar(metrics_mean.index, metrics_mean['ROC AUC'], yerr=metrics_std['ROC AUC'], label='ROC AUC', marker='^', capsize=5)
...
def plot2_metrics_over_time(all_metrics, T, path, save_img=False):
    ...
    # Plot mean and standard deviation for Specificity
    ...
    # Plot mean and standard deviation for Recall
    ...
    # Plot mean and standard deviation for ROC AUC
...
def plot_heatmap(attn_weights, features_list, X_test, split_num, patient_type):
    ...
    plt.xlabel('Features')
    plt.ylabel('Days')
    plt.title(f"Heatmap - {patient_type} - Train/Test Split {split_num}")
