...
sys.path.append("../")
from utils import *
from explainability_methods.att_method import *
...
def build_model_GRU(hyperparameters):
    ...
    weighted, hadamard_scores = hadamard_attention(masked, hyperparameters["layers"][0], hyperparameters['dropout'])
...
def build_model_LSTM(hyperparameters):
    """
    Builds a LSTM model with Hadamard attention based on several hyperparameters.
...
def build_model_Vanilla(hyperparameters):
    """
    Builds a Vanilla RNN model with Hadamard attention based on several hyperparameters.
    ...
    gru_encoder = tf.keras.layers.SimpleRNN(
        hyperparameters["layers"][1],
        dropout=hyperparameters['dropout'],
        return_sequences=True,
        activation=hyperparameters['activation'],
...
def run_network(X_train, X_val, y_train, y_val, 
    ...
    if hyperparameters["model_type"] == 'GRU':
        model = None
        model, hadamard_scores, dynamic_input = build_model_GRU(hyperparameters)
    
    elif hyperparameters["model_type"] == 'LSTM':
        model = None
        model, hadamard_scores, dynamic_input = build_model_LSTM(hyperparameters)
        
    elif hyperparameters["model_type"] == 'Vanilla':
        model = None
        model, hadamard_scores, dynamic_input = build_model_Vanilla(hyperparameters)
